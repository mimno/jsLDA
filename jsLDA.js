...
var documentsURL = QueryString().docs ? QueryString.docs : "projects.txt";
var stopwordsURL = QueryString().stoplist ? QueryString.stoplist :
  "sotu_stop.txt";
...
var reorderDocuments = function() {
  ...
  if (selectedTopic === -1) {
    ...
    var scores = documents.map(function(doc, i) {
      return {
        docID: i,
        score: (doc.topicCounts[selectedTopic] + docSortSmoothing) / (doc
          .tokens.length + sumDocSortSmoothing)
      };
    });
    ...
    d3.selectAll("div.document").data(scores)
      .style("display", function(d) {
        return documents[d.docID].topicCounts[selectedTopic] > 0 ? "block" :
          "none";
      })
      .text(function(d) {
...
function saveTopicKeys() {
  ...
  for (var topic = 0; topic < numTopics; topic++) {
    keysCSV += topic + "," + tokensPerTopic[topic] + ",\"" + topNWords(
      topicWordCounts[topic], 10) + "\"\n";
  }

  d3.select("#keys-dl").attr("href",
    "data:Content-type:text/csv;charset=UTF-8," + encodeURIComponent(keysCSV)
  );
}
...
function saveTopicPMI() {
  ...
  d3.select("#topictopic-dl").attr("href",
    "data:Content-type:text/csv;charset=UTF-8," + encodeURIComponent(pmiCSV));
}
...
function saveGraph() {
  ...
  d3.select("#graph-dl").attr("href",
    "data:Content-type:text/csv;charset=UTF-8," + encodeURIComponent(graphCSV)
  );
}
// 2016-03-07 OD: saveState doesn't appear to be called from anywhere //
function saveState() {
  ...
  d3.select("#state-dl").attr("href",
    "data:Content-type:text/csv;charset=UTF-8," + encodeURIComponent(state));
}
...
function ready(error, stops, lines) {
  if (error) {
    ...
    lines.split("\r").forEach(parseLine()); // 2016-03-04 OD: changed from FF to LF
    sortTopicWords();
    displayTopicWords();
